                                          ...
                                          bool    bOpenDriverSetup )
{
    ...
    // save number of channels from last driver (if they are set); this is done to check if we need to reset channel mapping
    ...
    if ( strStat.isEmpty() )
    {
        // Reset channel mapping if the number of channels has changed or the sound card name has changed
        if ( ( lNumInChanPrev != lNumInChan ) ||
             ( lNumOutChanPrev != lNumOutChan ) ||
             ( strCurDevName.compare ( strDriverNames[iDriverIdx] ) != 0 ) )
        {
            // TODO: In order to fix https://github.com/jamulussoftware/jamulus/issues/796
            // we reset the channel mapping if the number of channels has changed or the device
            // name has changed. This is not ideal.
